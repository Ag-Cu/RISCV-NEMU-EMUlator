...
#define immJ() do { *imm = (SEXT(BITS(i, 31, 21), 11) << 12) | (BITS(i, 20, 20) << 11) | (BITS(i, 19, 12) << 1) | (BITS(i, 20, 20) << 20); } while(0)
...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  INSTPAT("??????? ????? ????? ??? ????? 00101 11", auipc  , U, R(dest) = Pa(imm));
  INSTPAT("??????? ????? ????? 000 ????? 00100 11", addi   , I, R(dest) = Rw(imm, rs1));
  INSTPAT("??????? ????? ????? ??? ????? 11011 11", jal    , J, JUMP(imm, s));
