...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  INSTPAT("??????? ????? ????? 011 ????? 00100 11", sltiu  , I, Reg(dest) = SetIfCon(src1, SEXT(imm, 64), Sltiu));
  ...
  INSTPAT("0000001 ????? ????? 100 ????? 01110 11", divw   , R, Reg(dest) = SEXT(BITS(src1, 31, 0) / BITS(src2, 31, 0), 64));
  ...
  INSTPAT("000000? ????? ????? 001 ????? 00110 11", slliw  , I, Reg(dest) = SEXT(BITS(src1, 31, 0) << BITS(imm, 5, 0), 64));
  INSTPAT("010000? ????? ????? 101 ????? 00110 11", sraiw  , I, Reg(dest) = SEXT((int32_t)(BITS(src1, 31, 0)) >> BITS(imm, 5, 0), 64));
